...
- (void)setMinimumValue:(float)minimumValue {
    self.minLabel.text = [NSString stringWithFormat:@"%.2f", minimumValue];
...
- (void)setMaximumValue:(float)maximumValue {
    self.maxLabel.text = [NSString stringWithFormat:@"%.2f", maximumValue];
...
//    self.draw = NO;
	...
	self.minimumTrackTintColor = XHBlueColor;
    ...
    CGRect labelRect = CGRectMake((self.bounds.size.width - 50)/2, (self.bounds.size.height - 50)/2, 50, 50);
    self.valueLabel = [[UILabel alloc] initWithFrame:labelRect];
    ...
    CGRect minLabelRect = CGRectMake(CGRectGetMidX(labelRect) - radius*cos(45) - 40, CGRectGetMidY(labelRect) + radius*cos(45) + 30, 50, 20);
    CGRect maxLabelRect = CGRectMake(CGRectGetMidX(labelRect) + radius*cos(45) - 10, CGRectGetMidY(labelRect) + radius*cos(45) + 30, 50, 20);
    self.minLabel = [[UILabel alloc] initWithFrame:minLabelRect];
    self.maxLabel = [[UILabel alloc] initWithFrame:maxLabelRect];
...
- (void)drawValueAtPoint:(CGPoint)point inContext:(CGColorRef)contex
{
...
- (CGPoint)drawCircularTrack:(float)track atPoint:(CGPoint)center withRadius:(CGFloat)radius inContext:(CGContextRef)context {
	...
	float angleFromTrack = translateValueFromSourceIntervalToDestinationInterval(track, self.minimumValue, self.maximumValue, 0, 1.5*M_PI);
    ...
    //NSLog(@"startAngel: %f end: %f x: %f y: %f", startAngle, endAngle, arcEndPoint.x, arcEndPoint.y);
	return arcEndPoint;
}
...
- (void)panGestureHappened:(UIPanGestureRecognizer *)panGestureRecognizer {
	...
	switch (panGestureRecognizer.state) {
		case UIGestureRecognizerStateChanged: {
			...
			CGPoint sliderStartPoint = CGPointMake(sliderCenter.x - radius*cos(45), sliderCenter.y + cos(45));
			...
			self.value = translateValueFromSourceIntervalToDestinationInterval(angle, 0, 1.5*M_PI, self.minimumValue, self.maximumValue);
			break;
		}
